    ...
    async function renderCourses(filter = '') {
        ...
        if (courses.length > 0) {
            courses.forEach(course => {
                // Definiere den Inline-Style für die grüne Umrandung
                const courseStyle = course.studentCourse === "true" ? "border: 2px solid green;" : "";
                
                courseGrid.innerHTML += `
                    <div id="course_${course.courseNumber}" class="col mb-4" style="max-width: 300px;">
                        <div class="card h-100" style="width: 300px; ${courseStyle}">
